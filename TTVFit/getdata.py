...
class GetData():
    ...
    def getPaths(self):
        csvs = glob(self.indir+'*/'+'*white.csv')
    ...
    def getTime(self):
        ...
        time['ID'] = self.IDs
        if self.saveData:
            time.to_latex(self.indir+'time.tex', columns=['Median','+1 sigma','-1 sigma','ID'],index=False)
        return time
    ...
    def getHST(self,path):
        ...
        try:
            ...
            hst_time = pd.DataFrame({'T_mid':htime, 'Uncertainty':herror, 'Label':['HST']*len(htime), 'Reference':['Ma et al. 2024']*len(htime)})
            HST = True
        except:
            HST = False
            hst_time = None
    ...
    def getTESS(self,path1, path2, target):
        ...
        others_time = pd.DataFrame({'T_mid':others['T_mid'],'Uncertainty':others['Uncertainty (days)'],'Label':['Others']*len(others),'Reference':others['Reference']})
        try:
            tess = pd.read_csv(path2,names=['Orbit number','T_mid','Uncertainty (days)'])
            tess_time = pd.DataFrame({'T_mid':tess['T_mid'],'Uncertainty':tess['Uncertainty (days)'],'Label':['TESS']*len(tess),'Reference':['Wang et al. 2024']*len(tess)})

        except:
            tess = tess_all[tess_all['Reference']=='This work']
            tess_time = pd.DataFrame({'T_mid':tess['T_mid'],'Uncertainty':tess['Uncertainty (days)'],'Label':['TESS']*len(tess),'Reference':['Ivshina & Winn 2022']*len(tess)})
    ...
    def getT(self,hst_path, tess_path1, tess_path2, tess_target):
        ...
        if self.saveData:
            ...
            t.to_latex(self.indir+'t.tex', index=False)
        return t
    ...
    def checkDuplicates(self, t):
        ...
        if duplicate:
            ...
            if self.saveData:
                ...
                t_re.to_latex(self.indir+'t_re.tex', index=False)
            return t_re
        else:
            print('No duplicate.')