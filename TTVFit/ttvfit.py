...
class TTVFit:
    def __init__(self, data, period, parameters, parameter_names=[], kind='Linear', rej_sigma = None ,multiprocessing = False, limit=None, sampler='emcee',sampler_parameters=None):        
        """
    ...
    #         omega_N = (omega_0 + d_omega_dN*i) % (2*np.pi)
    #         t_tras[i] = t_tras[i-1] + period + e*P_ano/(np.pi)*d_omega_dN*np.sin(omega_N)
    #     return t_tras
    ...
    def apsidalPrecessionModel(self, params):
        ...
        params : list
            ...
            [t0, period, e, omega_0, d_omega_dN].
        ...
        omega_0 is the initial argument of periapsis, d_omega_dN is the rate
    ...
    def emceePrior(self, params):
        ...
        for i, par in enumerate(params):
            if self.parameters[i][-1]=='U':
                ...
                ValueError('Parameter type error. Should be U or N.')
    ...
    def Method(self):        
        ...
        if self.sampler == 'emcee':
            ...
            for i in range(len(self.parameters)):
                if self.parameters[i][-1]=='U':
                    pos.append(np.random.uniform(self.parameters[i][0], self.parameters[i][1], nwalkers))
                elif self.parameters[i][-1]=='N':
                    pos.append(np.random.normal(self.parameters[i][0], self.parameters[i][1], nwalkers))
                else:
                    ValueError('Parameter type error. Should be U or N.')
            ...
            self.samples = dynesty.utils.resample_equal(results.samples, weights)
        else:
            raise ValueError("Sampler not recognized")
        end = time.time()
    ...
    def runMethod(self,path=None):
        ...
        if (self.kind!='Linear'):
            self.lin = TTVFit(self.data.copy(),self.period,parameters=self.parameters[:2],rej_sigma=self.rej_sigma,multiprocessing=self.multiprocessing,limit=self.limit,sampler=self.sampler,sampler_parameters=self.sampler_parameters)
            self.lin.Method()
    ...
    def plotFit(self,path=None,ax=None,unitt='s'):
        ...
        if self.kind == 'Linear':
            ...
            for i,v in enumerate(self.data['Valid']):
                if v:
                    ax.errorbar(self.data.loc[i,'Epoch'], (self.data.loc[i,'T_mid']-self.lin.data.loc[i,'T_mid_pre'])*multi, yerr=self.data.loc[i,'Uncertainty']*multi, fmt='.',label=self.data.loc[i,'Label'],color=color_dict[self.data.loc[i,'Label']],ms=5,ecolor='dimgray',elinewidth=1,capsize=3)
            ax.errorbar(self.data[notValid]['Epoch'], (self.data[notValid]['T_mid']-self.lin.data[notValid]['T_mid_pre'])*multi, yerr=self.data[notValid]['Uncertainty']*multi, fmt='.',ms=5,ecolor='dimgrey',color='silver',alpha=0.5,elinewidth=1,capsize=3)
            ax.axhline(0, c='k', lw=1, ls='--')
    ...
    def test(self, outdir, suffix):
        ...
        for i in range(len(self.data['Epoch'])):
            ...
            main = TTVFit(data,self.period,parameters,self.parameter_names,self.kind,rej_sigma=self.rej_sigma,multiprocessing=self.multiprocessing,limit=self.limit,sampler=self.sampler,sampler_parameters=self.sampler_parameters)
            main.runMethod(path=outdir+'test_main%s_select%d'%(suffix,i))
            main.plotFit(path=outdir+'test_main%s_select%d'%(suffix,i),ax=ax[0])
            BICq = main.getBIC()
            lin = TTVFit(data,self.period,parameters[:2],self.parameter_names[:2],'Linear',rej_sigma=self.rej_sigma,multiprocessing=self.multiprocessing,limit=self.limit,sampler=self.sampler,sampler_parameters=self.sampler_parameters)
            ...
            ax[0].set_title(r'LOOCV Test (Select %d, $\Delta BIC$ = %.3f)'%(i,BICq-BIC0))
            plt.tight_layout()
    ...
    def Fit(self,outdir,suffix,title=None):
        ...
        with open(outdir+'prior_parameters.txt','w') as f:
        ...
        self.main = TTVFit(self.data.copy(),self.period,self.parameters,self.parameter_names,self.kind,rej_sigma=self.rej_sigma,multiprocessing=self.multiprocessing,limit=self.limit,sampler=self.sampler,sampler_parameters=self.sampler_parameters)
