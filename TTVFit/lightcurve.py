...
class LightCurve:
    ...
    def plotTransits(self,time):
        ...
        ax.errorbar(xdata, ydata, yerr=error,
            ms=15 , fmt='.', zorder=10, color='white',ecolor='dimgray',mec='skyblue',elinewidth=1,capsize=3, label='time')
        ax.tick_params(direction='in', which='both', labelsize=12)
        ax.xaxis.set_ticks_position('both')
        ax.yaxis.set_ticks_position('both')
        ax.set_xticks(xdata)
        ax.set_xlabel(r'$epoch$',fontsize=18)
        ax.set_ylabel(r'$t_0$[BJD-TDB]',fontsize=18)
        plt.tight_layout()
        if self.savePlots:
    ...
    def plotLightCurve(self,indir,t0,id=''):
        ...
        fig,ax = plt.subplots(2,1,figsize = (16,9),sharex=True, height_ratios=[3,1])

        if self.nbin is not None:
            ax[0].set_title(r'White Light Curve of %s (ID = %s, $t_0 = $ %.6f BJD-TDB, nbins = %s)'%(self.target,id,t0,self.nbin), fontsize=16)
            ...
            ax[0].errorbar(binned_t, binned_flux, yerr=binned_error, ms=15 , fmt='.',
                            color='white',ecolor='skyblue',mec='skyblue')
            ax[1].errorbar(binned_t, binned_residuals*1e6, yerr=binned_error*1e6,
                ms=15 , fmt='.', zorder=10,color='white',ecolor='dimgray',mec='skyblue',elinewidth=1,capsize=3)
        ...
        ax[1].set_xlabel('Time from Transit Midpoint [hrs]', fontsize=18)
    ...
    def plotLightCurveTogether(self, t0s, datas, ids):
        fig,ax = plt.subplots(figsize=(16, 9), facecolor='white', rasterized=False)
        offset = 0
        xpos = max([max(data['t']) for data in datas])*0.45
        for t0, data, id in zip(t0s, datas, ids):
            ...
            data['model'] -= center
            if self.nbin is not None:
                ax.set_title('White Light Curves of %s'%self.target, fontsize=16)
            ...
            ax.text(xpos,data['relative flux'].max()+offset, r'ID = %s, $t_0 = $%.6f'%(id,t0), fontsize=13)
        ax.set_ylabel('Relative Flux', fontsize=18)
        ax.set_xlabel('Time from Transit Midpoint [hrs]', fontsize=18)
